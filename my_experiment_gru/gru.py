...
def method_gru():
    ...
    if torch.cuda.is_available():
        gru_dilate = gru.my_gru(batch_size=2**7, train_set=loader_train, test_set=loader_test, n_result="001",
                                epochs=500, input_size=7, hidden_size=128, output_size=1, num_gru_layers=1,
                                optimizer="Adam", loss_func="dilate", lr=1e-4)
        gru_dilate.train_model()

        gru_mse = gru.my_gru(batch_size=2**7, train_set=loader_train, test_set=loader_test, n_result="001",
                             epochs=500, input_size=7, hidden_size=128, output_size=1, num_gru_layers=1,
                             optimizer="Adam", loss_func="mse", lr=1e-4)
        gru_mse.train_model()
